...
EOT;
    ...
    function __construct(){
        ...
        $this->outputfile = self::EXPORT_DIR."0UNU{$formattedDate}test.csv";
        $this->outputfilefinal = self::EXPORT_DIR."0UNU{$formattedDate}test.csv.pgp";
       
        //remove existing csv file
        $this->removeExistingCsvFile();
    ...
    public function encryptFile(){
        try {
            ...
            if (empty($keyInfo)) {
                $results = $pgp->import($this->publicKey);
...
public function uploadFileToSFtp()
{
    ...
    if ($pgpFiles && $this->fileStatus == 1) {
        ...
        try {
            ...
            if (!$sftp->put("/" . basename($latestFile), $latestFile, SFTP::SOURCE_LOCAL_FILE)) {
                ...
                $this->writeLogsOnTextFile("File uploaded successfully to /root");
            }
        } catch (Exception $e) {
    ...
    public function removeExistingCsvFile(){
        $csvFiles = glob(self::EXPORT_DIR.'*.csv*');
        try{
            if ($csvFiles) {
    ...
    public function writeCsvFile() {
        try {
            ...
            /*$subscriptions = $this->connection->get_results("SELECT * FROM sbr_subscription_details WHERE status = 1 AND user_id IN 
            ( SELECT user_id from wp_wc_user_changes WHERE timestamp BETWEEN NOW() - INTERVAL 1 DAY AND NOW() ) GROUP BY user_id, product_id");*/
            $subscriptions = $this->connection->get_results("SELECT * FROM sbr_subscription_details WHERE (STATUS = 1 OR next_order_date >= NOW()) AND shine_group_code != 0 AND (created_at BETWEEN NOW() - INTERVAL 1 DAY AND NOW() OR updated_at BETWEEN NOW() - INTERVAL 1 DAY AND NOW()) GROUP BY user_id, product_id");                    
            ...
            foreach ($subscriptions as $subscription) {
                ...
                //checks if user has un-subscribed and subscription last date is invalid
                if($subscription->status == 0 && $subscription->next_order_date <= date("Y-m-d")){
                    $otherSubscriptions = $this->connection->get_results("SELECT * FROM sbr_subscription_details WHERE user_id = $subscription->user_id AND status = 1 AND shine_group_code <> 0 ORDER BY id DESC");        
                    if(!$otherSubscriptions){
                        update_user_meta($subscription->user_id, 'is_shine_user', 0);
                        update_user_meta($subscription->user_id, 'shine_plan_code', 'N/A');
                        update_user_meta($subscription->user_id, 'shine_current_member_ship_plan', 'N/A');
                    }
                }
