...
function make_product_relationship() {
...
//$multiStage = array(8,9,10,11,20,21,22,34,35,36);
    ...
    $parentChilderns = array(
        ...
        '33' => array('34', '35', '36'),
        '38' => array('39'),
        '40' => array('41', '42', '43', '44', '45', '46'),
        '47' => array('48', '49', '50', '51', '52', '53'),
    );
    update_option('parentChilderns', $parentChilderns, 'yes');
    ...
    $posts_relation[40] = 711183;

    $posts_relation[41] = 711226;
    $posts_relation[42] = 711227;
    $posts_relation[43] = 711228;

    $posts_relation[44] = 711235;
    $posts_relation[45] = 711236;
    $posts_relation[46] = 711237;

    $posts_relation[47] = 711872;

    $posts_relation[48] = 711911;
    $posts_relation[49] = 711910;
    $posts_relation[50] = 711909;

    $posts_relation[51] = 711903;
    $posts_relation[52] = 711902;
    $posts_relation[53] = 711901;
    update_option('old_new_relation', $posts_relation, 'yes');
    //$saved_options = get_option('old_new_relation');
}

// import coupons
add_action('wp_ajax_insertMissingCoupons', 'insertMissingCoupons');

function insertMissingCoupons() {
    global $wpdb;
    //make_product_relationship();
    $offset = 22500;
    $limit = 40000;

    $sqlCoupons = "SELECT cop.*from coupon_  AS cop
	    LIMIT " . $offset . "," . $limit;
    $sqlCouponsNew = "SELECT cop.*FROM coupon_  AS cop WHERE cop.couponId NOT IN(SELECT couponId FROM coupon_ INNER JOIN wp_posts ON coupon_.couponCode = wp_posts.post_title)";
    $result = $wpdb->get_results($sqlCouponsNew, 'ARRAY_A') or die($wpdb->last_error);
    print_r($result);
...
add_action('wp_ajax_addOrderAddOns', 'addOrderAddOns');
...
function addOrderAddOns() {
    ...
    $sql = "SELECT orderAddOns.*, orderAddOns.authorizeTransactionId as authorizeTransactionIdaddon, order_.* from orderAddOns Join order_ on orderAddOns.orderId=order_.orderId where orderAddOns.ccResponse IN('approved','n/a') limit " . $offset . "," . $limit . "";
    echo $sql = "SELECT orderAddOns.*, orderAddOns.authorizeTransactionId as authorizeTransactionIdaddon, order_.* from orderAddOns Join order_ on orderAddOns.orderId=order_.orderId where orderAddOns.ccResponse IN('approved','n/a') AND orderAddOns.orderAddOnId NOT IN (SELECT orderAddOns.orderAddOnId FROM orderAddOns
...
add_action('wp_ajax_refundOrders', 'refundOrders');

function refundOrders() {
    global $wpdb;

    //$sql = "SELECT * from orderRefunds  limit ".$offset.",".$limit."";
    $sql = "SELECT orderRefunds.*, wp_postmeta.post_id FROM orderRefunds INNER JOIN wp_postmeta ON orderRefunds.orderId=wp_postmeta.meta_value WHERE wp_postmeta.meta_key='old_order_id' AND post_id NOT IN(SELECT post_parent  FROM wp_posts WHERE post_type='shop_order_refund')";
...
add_action('wp_ajax_updateORderss', 'updateORderss');

function updateORderss() {
    global $wpdb;
    $mysql_query3 = "SELECT post_id,meta_value FROM wp_postmeta INNER JOIN wp_posts ON wp_postmeta.post_id= wp_posts.ID WHERE meta_key ='_oldJson' AND post_status NOT IN('wc-refunded','wc-completed') ORDER BY ID DESC";
    $result3 = $wpdb->get_results($mysql_query3, 'ARRAY_A');
    $counter = 0;
    foreach ($result3 as $row) {
        ...
        if (isset($data["shipDate"])) {
            ...
            $already_updated = get_post_meta($pid, 'status_updated', true);
            if ($already_updated != 'yes') {
                ...
                update_post_meta($pid, 'status_updated', 'yes');
                echo $pid . '=>' . $orderStatus;
                echo '<br />';
                $counter++;
            }
        }
 ...
 */
add_action('wp_ajax_importOrder_top', 'importOrdersTop');
...
function importOrdersTop() {
    ...
    WHERE wp_postmeta.meta_key = 'old_order_id') ORDER BY orderId DESC";
    // die();
    ...
    if (count($next_arr) > 0) {
	   ...
	   where ordr.orderId IN(" . implode(',', $next_arr) . ") GROUP BY ordr.orderId ORDER BY couponDiscountTotal DESC";
...
function importOrderAddon($saved_options, $data) {
    ...
    $discount_total_addon = $data["addOnGelDiscount"] + $data["addOnDesenseGelDiscount"] + $data["addOnBrushHeadDiscount"];
    //'_customer_user';
    ...
    update_post_meta($order->id, 'old_order_id_addon', $data["orderAddOnId"]);
    update_post_meta($order->id, '_oldJson', json_encode($data));

    $authorizeTransactionId = $data['authorizeTransactionIdaddon'];

    if ($authorizeTransactionId != '') {

        update_post_meta($order->id, '_payment_method', 'authorize_net_cim_credit_card');
        update_post_meta($order->id, '_transaction_id', $authorizeTransactionId);
        //continue;
    }
    update_post_meta($order->id, '_cart_discount', $discount_total_addon);
    update_post_meta($order->id, 'order_type', 'Addon');
    ...
    if (count($result2) > 0) {
        ...
        if ($importFlage) {
            ...
            if (in_array($productIdOld, $multiStage)) {
                ...
                if ($data["shipDate"] != '' && $data["receiveDateImpressions"] == 0 && $data["shipDateTrays"] == 0) {
                    $orderStatus = 'wc-processing';
                }
                if ($data["shipDate"] != 0 && $data["receiveDateImpressions"] != 0 && $data["shipDateTrays"] == 0) {
                    $orderStatus = 'wc-partial_ship';
            ...
            if ($affirmCaptureId != '') {
                update_post_meta($order->id, '_payment_method', 'affirm');
                update_post_meta($order->id, '_transaction_id', $affirmCaptureId);
            }
            if ($orderContainsGift != '' && $orderContainsGift != 0) {
                update_post_meta($order->id, 'purchasing_as_giftset', 1);
            }
            if ($couponCode != '') {
...
//      
//                update_post_meta($order->id, 'coupon_applied', 'yes');
...
//                    $order->add_item($item);
            ...
            update_post_meta($order->id, '_order_shipping', $data['orderShippingTotal']);
            update_post_meta($order->id, '_cart_discount', $data["couponDiscountTotal"]);
            update_post_meta($order->id, '_order_tax', $data["orderSalesTaxTotal"]);
            update_post_meta($order->id, '_order_total', $data["orderTotal"]);
...
add_action('wp_ajax_updateCouponCodeLimits', 'updateCouponCodeLimits');

function updateCouponCodeLimits() {
    $parentChilderns = get_option('parentChilderns');
    $saved_options = get_option('old_new_relation');
    global $wpdb;
    $sql = "SELECT coupon_.*, wp_posts.ID FROM coupon_ INNER JOIN wp_posts ON coupon_.couponCode = wp_posts.post_title where post_type='shop_coupon'";
        ...
        $new_coupon_id = $row['ID'];
         if ($couponType == 'global') {
            ...
            if (isset($parentChilderns[$row['productId']])) {
                $coupon_products = $parentChilderns[$row['productId']];
                foreach ($coupon_products as $cp) {
                    $ids .= $saved_options[$cp] . ',';
                }
            } else {
                $ids = isset($saved_options[$row['productId']]) ? $saved_options[$row['productId']] : '0';
            }
            update_post_meta($new_coupon_id, 'individual_use', 'yes');
            update_post_meta($new_coupon_id, 'product_ids', $ids);
            if ($row['productQtyAvailable'] < 0) {
