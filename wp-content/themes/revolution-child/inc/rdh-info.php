...
if (!$user_id || $user_id == '') {
  $user_id = $wpdb->get_var( "SELECT ID FROM wp_users WHERE user_nicename = '".get_query_var('buddyname')."'" );
...
if (!$user_id || $user_id == '') {
  if ( is_email( get_query_var('buddyname') ) ) {
    $current_url  = $_SERVER['REQUEST_URI'];
    $userr = get_user_by( 'email', get_query_var('buddyname') );
    $user_id = $userr->id;
    $user_id_bp = '';
    $field_value = '';
    if(function_exists('bp_is_active')){
    ...
    if($user_id_bp !='') {
      $current_url = str_replace( get_query_var('buddyname'),$userr->user_login,$current_url);
