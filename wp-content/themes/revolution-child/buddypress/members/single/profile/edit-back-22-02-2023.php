               ...
               // $rdhc_video_ = '';
                ...
                ?>
                            <div class="referal-code">
                                <span><span class="referrsl-codeText">Invite Code:</span> <strong><? php // echo $rdhc_invite_code;
                                                                                                    ?></strong></span>
                            </div>
                <?php
    ...
    <form action="<?php bp_the_profile_group_edit_form_action(); ?>" method="post" id="profile-edit-form" class="standard-form profile-edit <?php bp_the_profile_group_slug(); ?>">
        <div class="container tabsWrapperParent">
            ...
            <div class="address-container">
                ...
                <div class="editfield degree_title">
                    <legend>State*</legend><select name="address[state]" required id="address-rep-state"><?php echo str_replace('<option value="' . $address_state . '">', '<option value="' . $address_state . '" selected>', $states_String); ?></select>
                </div>
            </div>
                    ...
                    <div class="field_wrapper experience">
                        ...
                        <?php if (is_array($experience) && count($experience) > 0) {
                            ...
                            for ($i = 0; $i < count($experience['exp_title']); $i++) {
                                ...
                                $replace_exp_employment_state = str_replace('<option value="' . $experience["state"][$i] . '">', '<option value="' . $experience["state"][$i] . '" selected>', $replace_exp_city);
...
<script type="text/javascript">
    ...
    jQuery(document).ready(function() {
        //  jQuery('.currently_Work').trigger('change');
        console.log('changed');
    });
    ...
    jQuery(document).on('click', '.mbtenddate', function() {
        endyear_val = jQuery(this).parents('.education-experiecce-wrapper').find('.mbtstartdate').val();
        jQuery(this).parents('.education-experiecce-wrapper').find('.mbtenddate').find('option').each(function() {
            option_val = jQuery(this).val();
            if (option_val < endyear_val) {
                jQuery(this).prop("disabled", true);
