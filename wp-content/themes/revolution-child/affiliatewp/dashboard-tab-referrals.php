...
<div id="affwp-affiliate-dashboard-referrals" class="affwp-tab-content">
	...
	<h4 class="hidden"><?php _e('Referrals', 'affiliate-wp'); ?></h4>
	...
	<table id="affwp-affiliate-dashboard-referrals" class="affwp-table affwp-table-responsive">
		<thead>
			<tr>
				<th class="referral-reference"><?php _e('Reference', 'affiliate-wp'); ?></th>
			...
			<?php if ($referrals) : ?>
				...
				<?php foreach ($referrals as $referral) : ?>
					<tr>
						...
						<td class="referral-description" data-th="<?php _e('Description', 'affiliate-wp'); ?>"><?php


																												if (isset($referral->products)) {
																													foreach ($referral->products as $productData) {

																														if (!has_term('raw', 'product_cat', $productData['id'])) {
																															echo wp_kses_post(nl2br($productData['name']));
																															echo '<br/>';
																														}
																													}
																												}
						...
						<td class="referral-date" data-th="<?php _e('Date', 'affiliate-wp'); ?>">
							<?php echo esc_html($referral->date_i18n('date')); ?>
							<div class="timeDisplayMbt">
								<?php echo esc_html($referral->date_i18n('time')); ?>
							</div>
						</td>
						<?php
						/**
