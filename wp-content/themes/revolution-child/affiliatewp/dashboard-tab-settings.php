...
?>
<?php if(isset($_REQUEST['affwp_action'])) {
	print_r($_POST);
	die();
update_user_meta($affiliate_user_id,'rdh_recommended_toggle_show',$_POST['rdh_recommended_toggle_show']);
}
?>
...
<div id="affwp-affiliate-dashboard-profile" class="affwp-tab-content">
	...
	<form id="affwp-affiliate-dashboard-profile-form" class="affwp-form" method="post">
				...
				<?php   $rdh_recommended_toggle_show = get_user_meta(get_current_user_id(),'rdh_recommended_url',true); ?>
				<div class="toggleButtonVideoToShowFrontpage">
                    <h4>Display your recommended products on your public profile page</h4>
                    <div class="toggleButtonVideoToShowFrontpageChild">
                        <div class="form-group-radio-custom">
                            <input type="radio" id="rdh_recommended_toggle_show" name="rdh_recommended_url" value="show" <?php if($rdh_recommended_toggle_show =='show'){ echo 'checked';}?>>
                            <label for="rdh_recommended_toggle_show">Show</label><br>
                         </div>   
                         <div class="form-group-radio-custom">
                            <input type="radio" id="rdh_recommended_toggle_hide" name="rdh_recommended_url" value="hide" <?php if($rdh_recommended_toggle_show =='hide'){ echo 'checked';}?>>
                            <label for="rdh_recommended_toggle_hide">Hide</label>
                         </div>   
                     </div>
                </div>

		<h4 class="hidden"><?php _e( 'Profile Settings', 'affiliate-wp' ); ?></h4>
