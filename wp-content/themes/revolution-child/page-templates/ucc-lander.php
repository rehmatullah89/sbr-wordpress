<?php
/*Template Name: Ucc sales Template */
global $wpdb;
global $GEHA_PAGE_ID;
...
if (isset($_GET['utm_campaign']) && $_GET['utm_campaign'] == 'userExists') {
    setcookie('ucc_user', 'yes', time() + (3600 * 24 * 360), '/');
    $_SESSION['ucc_user'] = 'yes';
    set_transient('ucc_user', 'yes', 365 * DAY_IN_SECONDS);
    if (is_user_logged_in()) {
        update_user_meta(get_current_user_id(), 'ucc_user', 'yes');
    }
    $uccFlag = true;
    if (function_exists('w3tc_flush_url')) {
        w3tc_flush_url(get_permalink(get_the_ID()));
...
if (isset($_COOKIE['ucc']) && $_COOKIE['ucc'] == 'yes') {
    $uccFlag = true;
}
if (isset($_SESSION['ucc_user']) && $_SESSION['ucc_user'] == 'yes') {
    $uccFlag = true;
}
if (isset($_COOKIE['ucc_userOneTime']) && $_COOKIE['ucc_userOneTime'] == 'yes') {
    $uccFlag = true;
    setcookie('ucc_userOneTime', '', time() - 100000, '/');
}
setcookie('ucc_user', 'yes', time() + (3600 * 24 * 360), '/');
$uccFlag = true;
?>
...
<style>
    ...
    #gehaPage #product-list.teethWhieteingSystemWrapper:not(.grid-changed) .medium-12.thb-dark-column.small-12.landing-product.child-5>.vc_column-inner {
        background-color:#d2e2d7 !important;
    }
...
#product-list .landing-product div[data-tagging="Featured!"] .featureTag{
    ...
    .uccDropDown li.depth-1.skin-careProductNav {
    padding-left: 22px;
}
    ...
    #product-list:not(.grid-changed) .medium-12.child-0 .product-selection-description  > .productDescriptionDiv.proTpDiv
    ,#product-list:not(.grid-changed) .landing-product.medium-12.child-18 .product-selection-description  > .productDescriptionDiv.proTpDiv
    ,#product-list:not(.grid-changed) .landing-product.medium-12.child-20 .product-selection-description  > .productDescriptionDiv.proTpDiv,
    #product-list:not(.grid-changed) .landing-product.medium-12.child-19 .product-selection-description  > .productDescriptionDiv.proTpDiv
    {
        display: none;
    }
...
.uccDropDown  .btn.btn-primary:hover{
    background-color: transparent !important;
}
ul.subMenuDropdown > li a:hover,#wrapper .subMenuDropdown li.depth-1.skin-careProductNav label:hover {
    color: #007ab3;
}
    .gehaTextmember {
        max-width: 286px;
    ...
    #product-list.grid-changed .landing-product .featureTag
    {
    margin-right: 3px;
    display: inline-flex;
    justify-content: center;
...
.enamel-subscription-product-box.one-time-offer .subscription-product-header{
...
#product-list .medium-12.landing-product.child-1 .product-selection-box .featureTag{
    ...
    @media (min-width: 991px) {
        .page-template-ucc-lander #product-list.productLandingPageContainer .landing-product:not(.medium-12) .product-selection-description b
    ,.page-template-ucc-lander  #product-list.grid-changed .caripro-product-outer h1
    ,.page-template-ucc-lander #gehaPage #product-list.grid-changed .medium-12 .product-selection-description b
    {
        font-size: 14px;
    ...
    @media (max-width: 767px) {
        ...
        .page-template-geha-template #product-list.productLandingPageContainer .medium-6 .discountedPriceForGehaMember, .page-template-geha-template #product-list.productLandingPageContainer .medium-6 .discountedPriceForGehaMember{
            bottom: -4px;
        }
...
html body  .uccDropDown button:hover {
    background-color: transparent;
...
html body  .uccDropDown .subMenuDropdown {
    position: absolute;
    top: 100%;
    left: 0;
    z-index: 1000;
...
section.shopLanderPageHader .pageheaderBotm{overflow: initial;background:#fff;}
...
ul.slideContent li label:hover {
...
.packageQuantityBox,.selectPackageBox{
    border-top: 1px solid #4a834b;
    border: 1px solid #4a834b;
}
.selectPackageBox{
    background-color: #d2e2d7 !important ;
...
.ucc-banner-wrap .aetna-logo-wrap{
    display: none;
}
html body .section-top-banner {
    padding-bottom: 30px;
...
.page-template-ucc-lander #wrapper  li.depth-1.skin-careProductNav .radio-custom-label:before
,.page-template-ucc-lander #wrapper  li.depth-1.skin-careProductNav .radio-custom-label:after
{
...
.main-teeth-img-custom{
    position: absolute;
    right: 15%;
    top: -23px;
}
.main-teeth-img-custom img {
    max-width: 325px;
    width: 325px;
    opacity:0.6;
}
...
section.shopLanderPageHader .flex-row {
    ...
    html body.page-template-ucc-lander .uccDropDown {
        max-width: 100%;
        width: 100%;
    }
    
    html body.page-template-ucc-lander .uccDropDown button, body.page-template-geha-template section.shopLanderPageHader .pageheaderBotm select {
        font-size: 14px;
    ...
    html body.page-template-ucc-lander .uccDropDown .dropdown-toggle::after, html body.page-template-geha-template .uccDropDown .dropdown-toggle::after {
    border-top: 0;
    border-right: 0;
    border-bottom: 0;
    border-left: 0;
}
html body.page-template-ucc-lander .uccDropDown .subMenuDropdown {
        min-width: 260px;
    }


    html body.page-template-geha-template .uccDropDown .subMenuDropdown {
...
html body.page-template-ucc-lander .subMenuDropdown>li>a, html body.page-template-ucc-lander .subMenuDropdown .skin-careProductNav, html body.page-template-ucc-lander #wrapper .subMenuDropdown label {
        font-size: 12px;
    }
    .main-teeth-img-custom img{
        max-width:150px;
    ...
    .page-template-ucc-lander #product-list.productLandingPageContainer .medium-12.thb-dark-column.small-12.landing-product >.vc_column-inner {
    background-color: #d2e2d7 !important;
}
    html body.page-template-ucc-lander section.shopLanderPageHader{
        background: #ffffff;
        border-bottom: 0px solid #ddebf2;
    }
    html body.page-template-ucc-lander .all-product-dropdown {
        width: 82%;
    }
    html body.page-template-ucc-lander  .disclaimer-bar-purple{
        margin-top: 0px;
    }
    .page-template-geha-template #product-list.productLandingPageContainer .medium-3 .discountedPriceForGehaMember{
        bottom: -5px !important;
    }
    html body.page-template-ucc-lander  #product-list.productLandingPageContainer .medium-12.thb-dark-column.small-12.landing-product >.vc_column-inner {
        background-color: #ffffff !important;
    }
...
<div class="gehaTemplate active-recomendation-tab" id="gehaPage">
    ...
    ?>
            ...
            <div class="salePageBannerHome">
                <?php
                get_template_part('inc/templates/ucc-saving-plans/shine-top-banner-member-page');
                ?>
            </div>
            ...
            <section class="shopLanderPageHader benefit-hub">
                ...
                <div class="pageheaderTop" style="background:linear-gradient(180deg, rgba(41,63,102,1) 0%, rgba(65,100,154,1) 100%);">
<div class="uucc-lander-flex">
<div class="row no-flex">
    <h1 class="font-mont weight-500 text-white"> <span class="united"style="font-weight:300">United</span> <span style="color: #fff;">Concordia</span> <br> <span style="text-transform: uppercase;"> <span style="color: #4a834b;">Member</span> Discounts </span></h1>
    <p class="open-sans text-white">Welcome member! Save up to 60% on Teeth Whitening & Oral Care!</p>
   
</div>
<div class="main-teeth-img-custom">
        <img src="<?php echo get_stylesheet_directory_uri(); ?>/assets/images/insurance-lander/tooth.png" alt="" data-src="<?php echo get_stylesheet_directory_uri(); ?>/assets/images/insurance-lander/tooth.png" decoding="async" class="lazyload">
    </div>
            </div>
                                ...
                                <nav>
                                    ...
                                    <ul class="subMenuDropdown">
                                        ...
                                        <li class="depth-1">
                                            ...
                                            <ul class="slideContent">
                                            <li class="depth-2">
                                                    <input type="radio" id="custom-whitening-trays-option-all" name="single-selection" class="child-radio" value="all whitenings" data-slug="all-teeth-whitenings">
                                                    <label for="custom-whitening-trays-option-all" class="radio-custom-label">All Teeth Whitenings</label>
                                                </li>
                                                <li class="depth-2">
            ...
            ?>
                            ...
                            <div class="rowMbt">
                                <div class="col-sm-4 geha-banner-text-section-parent">
                                    <div class="geha-banner-text-section  mt-100">
                                        <p class="font-mont text-white prpt ">PRODUCT 
                                            PARTNER OF</p>
                                        <img src="<?php echo get_stylesheet_directory_uri(); ?>/assets/images/tele-dentist-sale-lander/theteledentist-logo.png"
                                            alt="One Dental" class="ucc-logo-circle" />
                                        <div class="smtext">
                                            <div class="smtext-inr">
                                                <h2 class="font-mont fw-normal">
                                                    <span class="weight-lower-mbt">Exclusive discounts for The Ucc patients & their families.</span>
                                                </h2>
                                ...
                                <div class="whitening-teeth-girl-with-smile col-sm-8 graphicImagePeople">
                                    <div class="primary-head-image">
                                        <img src="<?php echo get_stylesheet_directory_uri(); ?>/assets/images/tele-dentist-sale-lander/tele-dentist-Image.png"
                                            alt=""
                                            data-src="<?php echo get_stylesheet_directory_uri(); ?>/assets/images/tele-dentist-sale-lander/tele-dentist-Image.png"
                    ...
                    <section class="ourCustomers">
                        <div class="container text-center">
                            <h2 class="font-mont" style="color:#2a2663;">OUR CUSTOMERS SPEAK FOR US</h2>
                            <h3>Over <span style="color:#2a2663">1 million happy customers</span> have relied on Smile Brilliant
                                to<br>
                        ...
                        <div class="section-body   light-blue clients-logoes text-center">
                            <div class="container" style>
                                ...
                                <div class="d-flex align-items-center justify-content-center logoes-strip-mbt mulimages">
                                    ...
                                    <span>
                                        <img src="<?php echo get_stylesheet_directory_uri(); ?>/assets/images/products/night-guards/round-icon-transparent.png"
                                            alt="" />
                                    </span>
                                </div>
                            </div>
                                ...
                                <div class="vc_column-inner   ">
                                    <div class="wpb_wrapper">
                                        ...
                                        <h3 style="text-align: center" class="vc_custom_heading">TRY IT. <span
                                                style="color:#2a2663;">LOVE IT</span> ...or return it</h3>
                                            ...
                                            <div class="widget widget_text">
                                                <div class="textwidget">
                                                    <p>Smile Brilliant is America's #1 brand of customized oral care products
                                                        and a
                                                        proud partner of Ucc Insurance. All Ucc clients, their
                                                        children, domestic
                                                ...
                                                <div class="textwidget">
                                                    ...
                                                    <div class="seeDiscountBtn text-center">
                                                        <button class="btn btn-primary-blue btn-lg scroll-link"
                                                            href="#gehaform">SEE Ucc DISCOUNTS</button>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                ...
                                <form id="geha_discount_registration_form" method="post">
                                    <div class="row form-steps fullWidth">
                                        ...
                                        <div class="product-of-intestest col-md-12">
                                            <h6 class="text-white">Products of Interest <span class="fw-normal"><i>(optional)</i></span></h6>
                                        ...
                                        <input type="hidden" name="source" value="ucc">
                                        ...
                                        <div class="col-md-12 text-center form-notes">
                                            <p class="text-white">By registering you will be able to bypass this page on future
                                                visits and view Ucc exclusive discounts so long as we recognize your devise
                                        ...
                                        <div class="gehaLoginCustomer w-100 text-center">
                                            <div class="registeded-btn-stp">
                                                <h3 class="font-mont text-white">ALREADY REGISTERED? <a href="/my-account/" style="color:#1cc2ea !important">Login</a> </h3>
            ...
            <?php if ($uccFlag) {
                echo '<style>.footer {padding: 0px 0;}</style>
                    ...
                    <div class="container">
                     <p class="postLoggedin text-white">“<strong>DISCLAIMER:</strong><i> In order to recieve uccmember exclusive pricing you must add to cart from this page.</i></p>
                    </div>
                    </div>';
            }
            ?>
        ...
        <div class="modal-content">
            ...
            <div class="popupBodyContent text-center">
                <h1 class="gehaTextmember">
                    <img src="<?php echo get_stylesheet_directory_uri(); ?>/assets/images/benefit-hub/uccmember.png" alt="uccmember" class="" />
                </h1>
                ...
                <p class="descriptionBody">Welcome uccmember member! You now have access to give your exclusive uccmember discounts. An email has also been sent to you with your account login details. To view your discounts in the future, simply login. Our live chat and customer support team is also here to help!<br><br>
                ...
                <div class=" sep-top-sm text-center seeDiscountBtn text-center">
                    <a href="<?php echo site_url('uccmember?user=yes'); ?>" class="btn">SEE DISCOUNT PAGE</a>
                </div>
            </div>
        </div>
    </div>
...
// }, 500);
    ...
    jQuery(document).ready(function() {
        jQuery('#resetButton').click(function() {
            ...
            jQuery(".uccDropDown").removeClass("menuActivate");
...
<script>
    ...
    jQuery(document).on('click', '#skipPartner', function(e) {
        ...
        setCookiePartner('uccmemberOneTime', 'yes', 200, '/');
        location.reload();
    });
        ...
        jQuery.ajax({
            ...
            success: function(response) {
                if (response.status == true) {
                    ...
                    setCookiePartner('uccmemberOneTime', 'yes', 2000, '/');
                    // location.reload();
                    // setTimeout(function() {
    ...
    jQuery(document).on('change', 'input[name="fav_language"]', function() {
        ...
        var ucc_sale_price = $(this).attr('data-ucc_sale_price');
        if (typeof ucc_sale_price !== 'undefined' && ucc_sale_price !== false && ucc_sale_price != '' && ucc_sale_price > 0) {
            jQuery(this).parents('.product-selection-box').find('.add_to_cart_button').attr('data-ucc_sale_price', ucc_sale_price);
        } else {
            jQuery(this).parents('.product-selection-box').find('.add_to_cart_button').removeAttr('data-ucc_sale_price');
        }
        jQuery(this).parents('.product-selection-box').find('.add_to_cart_button').attr('data-product_id', selected_product_id);
        jQuery(this).parents('.product-selection-box').find('.add_to_cart_button').attr('href', '?add-to-cart=' + selected_product_id);
        if (av_price != '') {
    ...
    //         $(".uccDropDown").toggleClass("menuActivate");
    //     });
    ...
    $(document).click(function(event) {
        if (!$(event.target).closest('.uccDropDown, #dropdownMenuButton').length) {
            $(".uccDropDown").removeClass("menuActivate");
        }
    });
