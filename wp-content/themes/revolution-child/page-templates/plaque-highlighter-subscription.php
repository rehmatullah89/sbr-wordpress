<?php
/*Template Name:  Geha Plaque Highlighter Landar demo */
$html = '';
...
if (isset($args['product_id'])) {
    ...
    $product_id = PLAQUE_HIGHLIGHTER1_PRODUCT_ID;
}
$plaquehighlighter_discount_data = [];
...
if($discount_type!='' && $discount_amount!=''){
    ...
    foreach($all_pids as $pidd){
            ...
            update_post_meta($pidd,$call_from_page .'_discounted_price_plaquehighlighter',$sale_price);
            $plaquehighlighter_discount_data[$pidd]=number_format($sale_price,2);
        }       
    }
...
.select-plaquehighlighter-content {
...
.select-plaquehighlighter-content select {
    border-radius: 15px;
    margin-bottom: 10px;
    line-height: 40px;
    height: 40px;
...
#wrapper .subscription-product-popup .select-plaquehighlighter-content label {
    display: none;
}
...
#wrapper .subscription-product-detail.subscription-product-popup .plaquehighlighter-subscription-product-box label {
    margin-bottom: 0;
    padding-right: 16px;
    padding-left: 36px;
...
#product-list:not(.grid-changed) .for-only-grid-view {
    display: none;
}
...
<div class="subscription-product-detail full-col plaqueHighlighterProuductWrapper">
    <div class="caripro-content ">
        <div class="caripro-content-splitter ">
            <div class="rating-stars d-flex align-items-center justify-content-center">
                <img class="mbl-img"
                    src="<?php echo get_stylesheet_directory_uri(); ?>/assets/images/products/plaque-highlighter-new/subscription-product.png" />
            ...
            <div class="caripro-product-outer">
                <h1>Plaque Highlighters</h1>
                <p class="desktop-img sub-heading-cari-pro">PROTECTS &amp; REPAIRS SENSITIVE TEETH</p>
                    ...
                    <div class="selectPackageBoxWrapper">
                        ...
                        <div class="subscription-product-inner-wrapper">
                            <div class="innershortpop">
                                <?php  
                        echo do_shortcode('[plaque-highlighter-subscriptions type="adults-plaque" pop="1" pids="'.PLAQUE_HIGHLIGHTER3_PRODUCT_ID.','.PLAQUE_HIGHLIGHTER2_PRODUCT_ID.','.PLAQUE_HIGHLIGHTER1_PRODUCT_ID.'" discount="'.$discount_type.','.$discount_amount.'"]');                         
                        ?>
                            </div>
    ...
    <div class="fruit-content">
        ...
        <div class="middile-content">
            <p>Reduces plaque &amp; tartar build-up</p>
            <p>Improves brushing technique</p>
            <p class="berry-falour">Supports overall oral health</p>
            <img class="" src="https://smilebrilliant.com/wp-content/themes/revolution-child/assets/images/products/plaque-highlighter-new/tbtlts-middle-img.png">
        </div>
    ...
    <div class="subscription-product">
        <div class="subscription-product-inner-wrapper">
            <?php
           update_option('plaquehighlighter_discounts',$plaquehighlighter_discount_data);
            echo do_shortcode('[plaque-highlighter-subscriptions type="adults-plaque" pids="'.PLAQUE_HIGHLIGHTER3_PRODUCT_ID.','.PLAQUE_HIGHLIGHTER2_PRODUCT_ID.','.PLAQUE_HIGHLIGHTER1_PRODUCT_ID.'" discount="'.$discount_type.','.$discount_amount.'"]');
            ?>
        </div>
...
$(document).ready(function() {
    ...
    $('input[name="radio-group-plaque"]:checked').closest('.plaquehighlighter-subscription-product-box').find(
        '.subscription-product-content-body').show();

    // Add event listener to radio buttons with the name "radio-group-plaque"
    $('input[name="radio-group-plaque"]').change(function() {

        // Hide all subscription-product-content-body elements
        $(this).closest('.subscription-product-inner-wrapper').find(
            '.subscription-product-content-body').hide();

        // Get the closest subscription-product-content-body element
        var contentBody = $(this).closest('.plaquehighlighter-subscription-product-box').find(
            '.subscription-product-content-body');
        ...
        $(this).closest('.subscription-product-inner-wrapper').find('.subscription-product-header')
    ...
    $('.plaquehighlighter-subscription-product-box').each(function() {
        var productBox = $(this);

        var product_id = productBox.find(
            '.select-plaquehighlighter-content select[name="subc_quantity"]').val();
        var freq = productBox.find('.select-plaquehighlighter-content select[name="subc_frequency"]')
            .val();
        var discount = $('#discount_amt_plaquehighlighter').val();
        var disc_type = $('#discount_type_plaquehighlighter').val();

        if (productBox.find(
                '.select-plaquehighlighter-content select[name="subc_quantity"] option:selected').attr(
                "on-sale") == 1) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                productBox.find("#plaquehighlighter_subscription_item_data").val(response);
                        ...
                        $.each(prodObj, function(key, value) {
                            if (discount > 0) {
                                if (disc_type == 'fixed') {
                                    ...
                                    value.price = (new_price > 0 ?
                                        new_price : value.price);
                                } else if (disc_type == 'percentage') {
                                    let percent_disc = (value.price * (
                                        discount / 100));
                            ...
                            if (options == '') {
                                ...
                                setupPrice(productBox, "P2", value.arbid,
                                    value.price, product_id, value
                                    .reg_price, value.frequency,
                                    discount, disc_type);
                            }
                ...
                var percent = (new_price > 0 && old_price > 0) ? ((100 - (new_price /
                    old_price) * 100)).toFixed(2) : 0;

                productBox.find("#price_today").html(productBox.find("#P2_price").html());
                var period = "";
                if (freq > 7 && freq <= 31) {
                    period = "mo"
                ...
                productBox.find("#after_price").html(productBox.find("#P2_price").html() +
                    ' /' + period);
                productBox.find("#save_price").html(updated_price.toFixed(2));
                productBox.find("#save_percent").html(percent);
                productBox.find(
                        '.select-plaquehighlighter-content select[name="subc_frequency"]')
                    .html(options);
            },
            error: function(xhr) {},
            cache: false,
...
function setupPrice(productBox, btnId, arbId, price, product_id, reg_price, frequency, discount, disc_type) {
...
function updatePlaqueOrderSubscriptionItem() {
    console.log("im here 3");
    var productBox = $(this).closest('.plaquehighlighter-subscription-product-box');
    var product_id = productBox.find('.select-plaquehighlighter-content select[name="subc_quantity"]').val();
    var freq = productBox.find('.select-plaquehighlighter-content select[name="subc_frequency"]').val();
    var response = productBox.find("#plaquehighlighter_subscription_item_data").val();
    var discount = $('#discount_amt_plaquehighlighter').val();
    var disc_type = $('#discount_type_plaquehighlighter').val();
    var options = '';
    var obj = JSON.parse(response);
        ...
        if (prod == product_id) {
            $.each(prodObj, function(key, value) {
                ...
                if (freq == value.frequency) {
                    setupPrice(productBox, "P2", value.arbid, value.price, product_id, value.reg_price,
                        value.frequency, discount, disc_type);
                }
    ...
    if (productBox.find('.select-plaquehighlighter-content select[name="subc_quantity"] option:selected').attr(
            "on-sale") == 1) {
        productBox.find("#sale_div2").show();
    } else {
        productBox.find("#sale_div2").hide();
    ...
    let updated_price = old_price - new_price;
    productBox.find("#after_price").html(productBox.find("#P2_price").html() + ' /' + period);
    productBox.find("#save_price").html(updated_price.toFixed(2));
    productBox.find("#save_percent").html(percent);

    productBox.find('.select-plaquehighlighter-content select[name="subc_frequency"]').html(options);
}

function addPlaqueOrderSubscriptionItem() {
    var productBox = $(this).closest('.plaquehighlighter-subscription-product-box');
    var product_id = productBox.find('.select-plaquehighlighter-content select[name="subc_quantity"]').val();
    var freq = productBox.find('.select-plaquehighlighter-content select[name="subc_frequency"]').val();
    var discount = $("#discount_amt_plaquehighlighter").val();
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            productBox.find('.select-plaquehighlighter-content select[name="subc_frequency"]').html(data
                .options);
            ...
            setupPrice(productBox, "P2", arbId, price, product_id, reg_price, frequency, discount,
                disc_type);
        },
        error: function(xhr) {},
        cache: false,
...
function togglePricePlaque(btnId, selectValue, selectElement) {
    var plaquehighlighter_discounted_prices = '<?php echo json_encode($plaquehighlighter_discount_data);?>';
    var productBox = $(selectElement).closest('.plaquehighlighter-subscription-product-box');
    console.log("DISC PRICE Plaque:", plaquehighlighter_discounted_prices);
    console.log(plaquehighlighter_discounted_prices);
    console.log(productBox);
    console.log(selectValue);
    console.log($(selectElement).val());
    var jsonObject = JSON.parse(plaquehighlighter_discounted_prices);
    ...
    if (jsonObject.hasOwnProperty(keyToCheck)) {
        ...
        var val = $(selectElement).parents('.product-selection-box').find("input[name='product_name" + selectValue +
            "']").val();
    }

    var old_price = $(selectElement).parents('.product-selection-box').find("input[name='product_name" + selectValue +
        "']").attr("data-old_price");
    ...
    if (productBox.find("#Quantity").find("option:selected").attr("on-sale") == 1) {
        productBox.find("#sale_div_plaquehighlighter").show();
    } else {
        productBox.find("#sale_div_plaquehighlighter").hide();
    }
    productBox.find("#" + btnId).attr("data-" + pslug + "_user", "yes");
    productBox.find("#" + btnId).attr("data-" + pslug + "_plaque_price", val);
...
$(document).on('change',
    '.select-plaquehighlighter-content select[name="subc_quantity"], .select-plaquehighlighter-content select[name="subc_frequency"]',
    updatePlaqueOrderSubscriptionItem);
//$(document).on('click', '.add-to-cart-btn button', addPlaqueOrderSubscriptionItem);
</script>
<script>
...
// Ensure the radio button is checked on page load
