<?php
/*Template Name:  Geha Enamel Armour Landar demo */
$html = '';
...
if (isset($args['product_id'])) {
    ...
    $product_id = ENAMELARMOUR_1TUBE_PRODUCT_ID;
}
$enamel_discount_data = [];
...
if($discount_type!='' && $discount_amount!=''){
    ...
    foreach($all_pids as $pidd){
            ...
            update_post_meta($pidd,$call_from_page .'_discounted_price_enamelarmour',$sale_price);
            $enamel_discount_data[$pidd]=number_format($sale_price,2);
        }       
    }
...
<style>
    ...
    #wrapper .subscription-product-popup .select-enamel-content label {
        display: none;
    }
    ...
    #wrapper .subscription-product-detail.subscription-product-popup .enamel-subscription-product-box label {
        margin-bottom: 0;
        padding-right: 16px;
        padding-left: 36px;
...
<div class="subscription-product-detail full-col enamelArmourProuductWrapper">
   <div class="caripro-content ">
            ...
            <div class="selectPackageWrapper subscription-product-popup subscription-product-detail">
                <div class="selectPackageBox">
                    <div class="selectPackageBoxWrapper">
                        ...
                        <?php  
                        echo do_shortcode('[enamel-armour-subscriptions type="enamel-armour" pop="1" pids="'.ENAMELARMOUR_3TUBE_PRODUCT_ID.','.ENAMELARMOUR_2TUBE_PRODUCT_ID.','.ENAMELARMOUR_1TUBE_PRODUCT_ID.'" discount="'.$discount_type.','.$discount_amount.'"]');                         
                        ?>
                        </div>
            ...
            <div class="header-content">
             <p class="weight-600">See noticeable results in just 60 days or return it for a full refund.</p>
                ...
                <div class="middile-content">
                        ...
                        <img class="" src="https://asif.smilebrilliant.com/wp-content/themes/revolution-child/assets/images/products/enamel-armour-subscription/leaf-image.jpg">
    ...
    <div class="subscription-product">
        <div class="subscription-product-inner-wrapper">
            <?php
           update_option('enamelarmour_discounts',$enamel_discount_data);
            echo do_shortcode('[enamel-armour-subscriptions type="enamel-armour" pids="'.ENAMELARMOUR_3TUBE_PRODUCT_ID.','.ENAMELARMOUR_2TUBE_PRODUCT_ID.','.ENAMELARMOUR_1TUBE_PRODUCT_ID.'" discount="'.$discount_type.','.$discount_amount.'"]');
            ?>
        </div>
...
<script>
    ...
    $('input[name="radio-group-enamal"]:checked').closest('.enamel-subscription-product-box').find('.subscription-product-content-body').show();
    ...
    $('input[name="radio-group-enamal"]').change(function () {
        ...
        var contentBody = $(this).closest('.enamel-subscription-product-box').find('.subscription-product-content-body');
    ...
    $('.enamel-subscription-product-box').each(function () {
        var productBox = $(this);

        var product_id = productBox.find('.select-enamel-content select[name="subc_quantity"]').val();
        var freq = productBox.find('.select-enamel-content select[name="subc_frequency"]').val();
        var discount = $('#discount_amt_enamel').val();
        var disc_type = $('#discount_type_enamel').val();

        if (productBox.find('.select-enamel-content select[name="subc_quantity"] option:selected').attr("on-sale") == 1) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                productBox.find("#enamel_subscription_item_data").val(response);
                    ...
                    if (prod == product_id) {
                        $.each(prodObj, function (key, value) {
                            ...
                            if (options == '') {
                                ...
                                setupPrice(productBox, "P2", value.arbid, value.price, product_id, value.reg_price, value.frequency, discount, disc_type);
                            }
                ...
                productBox.find('.select-enamel-content select[name="subc_frequency"]').html(options);
            },
            error: function (xhr) {},
            cache: false,
        });
...
function updateEnamelOrderSubscriptionItem() {
    ...
    var productBox = $(this).closest('.enamel-subscription-product-box');
    var product_id = productBox.find('.select-enamel-content select[name="subc_quantity"]').val();
    var freq = productBox.find('.select-enamel-content select[name="subc_frequency"]').val();
    var response = productBox.find("#enamel_subscription_item_data").val();
    var discount = $('#discount_amt_enamel').val();
    var disc_type = $('#discount_type_enamel').val();
    var options = '';
    var obj = JSON.parse(response);
...
function addEnamelOrderSubscriptionItem() {
    var productBox = $(this).closest('.enamel-subscription-product-box');
    var product_id = productBox.find('.select-enamel-content select[name="subc_quantity"]').val();
    var freq = productBox.find('.select-enamel-content select[name="subc_frequency"]').val();
    var discount =  $("#discount_amt_enamel").val();
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            productBox.find('.select-enamel-content select[name="subc_frequency"]').html(data.options);
            ...
            setupPrice(productBox, "P2", arbId, price, product_id, reg_price, frequency, discount, disc_type);
        },
        error: function (xhr) {},
        cache: false,
    });
...
function togglePriceEnamel(btnId, selectValue, selectElement) {
    var enamel_discounted_prices = '<?php echo json_encode($enamel_discount_data);?>';
    var productBox = $(selectElement).closest('.enamel-subscription-product-box');
    console.log("DISC PRICE Enamel:",enamel_discounted_prices);
    console.log(enamel_discounted_prices);
...
if (jsonObject.hasOwnProperty(keyToCheck)) {
    ...
    if (productBox.find("#Quantity").find("option:selected").attr("on-sale") == 1) {
        productBox.find("#sale_div_enamel").show();
    } else {
        productBox.find("#sale_div_enamel").hide();
    }
        productBox.find("#" + btnId).attr("data-"+pslug+"_user", "yes");
        productBox.find("#" + btnId).attr("data-"+pslug+"_enamel_price", val);
...
$(document).on('change', '.select-enamel-content select[name="subc_quantity"], .select-enamel-content select[name="subc_frequency"]', updateEnamelOrderSubscriptionItem);
//$(document).on('click', '.add-to-cart-btn button', addEnamelOrderSubscriptionItem);
