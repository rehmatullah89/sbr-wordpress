...
function total_upsell_mbt_back($params = array())
{
    ...
    $sql = "SELECT P.post_status ,WOI.order_item_id  , WOIM.meta_key , WOIM.meta_value  , WOI.order_id FROM wp_posts P INNER JOIN wp_woocommerce_order_items as WOI ON P.ID= WOI.order_id INNER JOIN wp_woocommerce_order_itemmeta as WOIM ON WOI.order_item_id= WOIM.order_item_id where post_type='shop_order' AND  P.post_status IN ('wc-completed' , 'wc-partial_ship' , 'wc-shipped' , 'wc-on-hold' , 'wc-processing', 'wc-pending', 'wc-wfocu-pri-order', 'wc-partial-refunded')  AND WOIM.meta_key IN('_qty','_product_id' , '_upsell_source')";
    if ($order_start_date != '' && $order_end_date != '') {
        $order_start_date = $order_start_date . ' 00:00:00';
        $order_end_date = $order_end_date . ' 23:59:59';
        $sql .= " AND P.post_date BETWEEN '$order_start_date' AND '$order_end_date '";
...
function total_number_of_orders_geha_new_rev_sbr_report($params)
{
    ...
    $sql = "select  P.ID , PM.meta_key , PM.meta_value from wp_posts as P INNER JOIN wp_postmeta as PM ON P.ID=PM.post_id INNER JOIN wp_postmeta as PM2 ON P.ID=PM2.post_id where post_type='shop_order' AND  post_status IN ('wc-completed' , 'wc-partial_ship' , 'wc-shipped' , 'wc-on-hold' , 'wc-processing', 'wc-pending', 'wc-wfocu-pri-order', 'wc-partial-refunded') AND PM.meta_key='_customer_user'";
...
function total_number_of_orders_rev_sbr_report($params)
{
    ...
    $order_start_date = isset($params['start_date']) ? date('Y-m-d', strtotime($params['start_date'])) : '';
    $order_end_date = isset($params['end_date']) ? date('Y-m-d', strtotime($params['end_date'])) : '';
    $sql = "select P.ID,PM.meta_key,PM.meta_value from wp_posts as P INNER JOIN wp_postmeta as PM ON P.ID=PM.post_id where P.post_type='shop_order'  AND P.post_status IN ('wc-completed' , 'wc-partial_ship' , 'wc-shipped' , 'wc-on-hold' , 'wc-processing', 'wc-pending', 'wc-wfocu-pri-order', 'wc-partial-refunded') AND PM.meta_key IN('_order_total','_order_shipping','_cart_discount','_wc_cog_order_total_cost','hfa_hsa','_shipping_state')";
    $sql2 = "select IFNULL(SUM(PM.meta_value),0) as refunds from wp_posts as P INNER JOIN wp_postmeta as PM ON P.ID=PM.post_id where P.post_type='shop_order' AND P.post_status = 'wc-refunded' AND PM.meta_key = '_bwf_customer_refunded' ";
    ...
    if ($order_type != '') {
        if ($order_type == 'local') {
            $sql2 .= " AND P.ID IN (select P.ID from wp_posts as P INNER JOIN wp_postmeta as PM ON P.ID=PM.post_id where P.post_type='shop_order' AND P.post_status = 'wc-refunded' AND PM.meta_key = '_billing_country' AND PM.meta_value = 'US' {$joinSql}) ";
        }elseif ($order_type == 'international') {
            $sql2 .= " AND P.ID IN (select P.ID from wp_posts as P INNER JOIN wp_postmeta as PM ON P.ID=PM.post_id where P.post_type='shop_order' AND P.post_status = 'wc-refunded' AND PM.meta_key = '_billing_country' AND PM.meta_value != 'US' {$joinSql}) ";
        }
    }
    $easypostCost = 0;
    ...
    if (count($results) > 0) {
        ...
        foreach ($results as $res) {
            ...
            if(in_array($res['meta_key'], ['_cart_discount', '_order_shipping', '_wc_cog_order_total_cost', '_order_total']) && gettype($res['meta_key']) == 'string'){
                $error_orders[$res['ID']] = $res['ID'];
            }
            if ($res['meta_key'] == '_cart_discount') {
                $_cart_discount += (float)$res['meta_value'];
            }
...
function total_revenue_by_date_mbt($order_start_date, $order_end_date, $order_type)
{
    ...
    global $wpdb;
    $sql = "select P.ID,PM.meta_key,PM.meta_value from wp_posts as P INNER JOIN wp_postmeta as PM ON P.ID=PM.post_id where P.post_type='shop_order'  AND P.post_status IN ('wc-completed' , 'wc-partial_ship' , 'wc-shipped' , 'wc-on-hold' , 'wc-processing', 'wc-pending', 'wc-wfocu-pri-order', 'wc-partial-refunded') AND PM.meta_key IN ('_order_total','_order_shipping','_cart_discount','_wc_cog_order_total_cost')";
    if ($order_start_date != '' && $order_end_date != '') {
        $order_start_date = $order_start_date . ' 00:00:00';
        $order_end_date = $order_end_date . ' 23:59:59';
        $sql .= ' AND P.post_date BETWEEN "' . $order_start_date . '" AND "' . $order_end_date . '"';
...
AND wp_woocommerce_order_itemmeta.meta_key="_product_id" AND wp_woocommerce_order_itemmeta.meta_value IN("' . $product_id . '") AND wp_posts.post_status IN ("wc-completed" , "wc-partial_ship" , "wc-shipped" , "wc-on-hold" , "wc-processing", "wc-pending", "wc-wfocu-pri-order", "wc-partial-refunded")  )';
        $results = $wpdb->get_results($sql, 'ARRAY_A');
        $reportData = array();
